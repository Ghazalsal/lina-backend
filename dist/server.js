...
app.get("/api/users/search", asyncHandler(async (req, res) => {
    ...
    let user;
    if (phone)
        user = await usersDB.findOne(u => u.phone === String(phone).trim());
    else if (id)
        user = await usersDB.getById(String(id));
    if (!user) {
        res.status(404).json({ error: "User not found" });
        return;
    }
...
app.put("/api/appointments/:id", asyncHandler(async (req, res) => {
    ...
    if (type) {
        updates.type = type;
        updates.duration = ServiceDurations[type] ?? appointment.duration;
    }
    if (time) {
        const startTime = new Date(time);
...
app.post("/api/appointments/:id/send-whatsapp", asyncHandler(async (req, res) => {
    ...
    const sent = await sendWhatsAppMessage(user.phone, user.name, dateStr, appointment.time, service, dayName, lang);
    if (sent) {
